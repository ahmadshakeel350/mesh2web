<?php echo view('db_head'); ?>
          ...
          <div class="row">
            <div class="col-sm-4">
              <form class="mb-3" action="" id="birthday_template_form" enctype="multipart/form-data"  method="POST" autocomplete="off">
                ...
                <div class="form-group mb-4">
                  <div class="d-flex flex-row justify-content-between">
                    <label for="" class="form-label label-form">Person's Name</label>
                  </div>
                  <div class="search-input">
                    <input
					  ...
					  id="person_name"
					  name="person_name"
					  value="<?php echo $templateSettings['person_name'];?>"
                    />
                  </div>
                </div>
	...
	<script>
		...
		{
			console.log("Sending Birthday Update:"+pname);
			message = new Paho.MQTT.Message(pname);
			message.destinationName = "/signage/birthday/"+tid;
			mqtt.send(message);
		}
            ...
            $.ajax({
              ...
              success: function (data) {
                  if(tid==='none')
                  {
                      ...
                      document.getElementById('display_screen').src = "<?= base_url(); ?>/bd/"+tid;
        		  }
        		  template = document.getElementById('person_name').value;
        		  sendUpdate(template);
                  console.log(data);
              },
              error: function (data) {
